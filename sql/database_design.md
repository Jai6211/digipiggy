...
## 1) Features → Data Mapping
| Feature | Entities | Key Data |
|---|---|---|
| Sign up / login | users, auth_providers, sessions | email, password_hash, provider_uid, jwt_id |
| Wallet account | wallet_accounts | owner_type=user, owner_id, currency, available_cents |
| Deposit/Withdraw | transactions | wallet_account_id, type, amount_cents, status |
| Round-up | receipts, roundups, transactions | purchase_cents, roundup_cents, strategy |
| Merchant 5% | change_contributions, merchant_commissions | contribution_id, commission_rate_bps=500 |
| Group/Pot | groups, group_members, pots, pot_members | roles, visibility, 1:1 pot↔wallet_account |
| Goals | savings_goals, goal_progress | target_cents, target_date, daily snapshot |
| Notifications/Badges | notifications, badges, user_badges, notification_settings | channel, read, badge criteria |

---

## 2) Entities & Constraints (MVP)
- **users**: `id PK`, `email UNIQUE NOT NULL`, `status ENUM`, timestamps  
- **wallet_accounts**: `id PK`, `(owner_type, owner_id)`, `available_cents`  
- **transactions**: `id PK`, `wallet_account_id FK`, `user_id FK`, `type ENUM`, `amount_cents`  
- **merchants**: `id PK`, `name UNIQUE`  
- **receipts**: `id PK`, `user_id FK`, `merchant_id FK NULL`, `total_cents`, `purchased_at`  
- **roundups**: `id PK`, `user_id FK`, `receipt_id FK NULL`, `purchase_cents`, `roundup_cents CHECK 0..99`  

Full DDL: [`schema/phase2_digipiggy.sql`](schema/phase2_digipiggy.sql)
...
```mermaid
erDiagram
  users ||--o{ wallet_accounts : "owns via owner_type=user"
  users ||--o{ transactions : "makes"
  users ||--o{ receipts : "submits"
  wallet_accounts ||--o{ transactions : "posts"
  receipts }o--|| merchants : "at"
  users ||--o{ roundups : "gets"
  receipts ||--o{ roundups : "optional"

  groups ||--o{ group_members : ""
  users ||--o{ group_members : ""
  groups ||--o{ pots : ""
  pots ||--|| wallet_accounts : "1:1 pot account"
  pots ||--o{ pot_members : ""
  pots ||--o{ savings_goals : ""
  savings_goals ||--o{ goal_progress : ""


flowchart LR
  actorUser([User/Student])
  actorMerchant([Shopkeeper])
  actorAdmin([Admin])

  subgraph System[DigiPiggy]
    UC1([Sign up / Login])
    UC2([Create Wallet Account])
    UC3([Deposit / Withdraw])
    UC4([Round-up from Receipt])
    UC5([View Balance & Transactions])
    UC6([Join Group Pot])
    UC7([Merchant Incentive 5%])
    UC8([Notifications & Badges])
  end
